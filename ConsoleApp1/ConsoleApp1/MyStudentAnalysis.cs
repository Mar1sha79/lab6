...
public static class MyStudentAnalysis
{
    ...
    public static void AddStudentThroughConsole()
    {
        ...
        try
        {
            ...
            // Запись в файл
            using (StreamWriter writer = new StreamWriter(StudentsFileName, true, Encoding.UTF8))
            {
                writer.WriteLine(studentData);
            }

            Console.WriteLine($"\nУченик успешно добавлен");
            Console.WriteLine($"Запись: {studentData}");
        }
        catch (Exception ex)
        {
